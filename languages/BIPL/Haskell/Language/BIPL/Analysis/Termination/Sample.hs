...
decrementToZero =
  Seq (Assign "x" (IntConst 10))
      (While (Binary Geq (Var "x") (IntConst 1))
...
incrementToLimit =
  Seq (Assign "i" (IntConst 0))
      (While (Binary Lt (Var "i") (IntConst 10))
...
movesAwayFromBound =
  While (Binary Gt (Var "x") (IntConst 0))
...
-- | Unknown: the guard is Boolean rather than a recognized variable/bound comparison.
...
unknownConditionalUpdate =
  While (Binary Lt (Var "x") (IntConst 10))
        (If (Var "b")
            (Assign "x" (Binary Add (Var "x") (IntConst 1)))
